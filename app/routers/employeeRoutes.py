...
from ..schemas import EmployeeBase,EmployeeUpdateBase # Pydantic Schema
...
def update_employee(emp_id: int, employee: EmployeeUpdateBase, db: Session = Depends(get_db_dependency)):
    try:
        existing_employee = db.query(EmployeeTable).filter(EmployeeTable.emp_id == emp_id).first()
